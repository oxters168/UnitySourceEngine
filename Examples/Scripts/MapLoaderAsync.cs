...
{
    public class MapLoaderAsync : MonoBehaviour
    {
        ...
        public BSPMap LoadMap(string mapLoc, bool combineMeshesWithSameTextures = true, float faceLoadPercent = 1, float modelLoadPercent = 1, bool flatTextures = false, int maxTextureSize = 2048)
        {
            ...
            loadingTask = UnityHelpers.TaskManagerController.RunActionAsync("Parsing Map", (cancelToken) => { map.ParseFile(cancelToken, null, () =>
            {
                if (!loadingTask.cancelled)
                    UnityHelpers.TaskManagerController.RunAction(() => { map.MakeGameObject(null, (go) => { go.SetActive(true); loadingBar.transform.root.gameObject.SetActive(false); }); });
            }); });
